...
$this->breadcrumbs=array(
	...
	Catalogsys::model()->getCatalog('country'));
...
function adddata()
{
	<?php echo CHtml::ajax(array(
		...
		'success'=>"function(data)
		{
			...
			{
				$('#Country_countryid').val('');
$('#Country_countrycode').val('');
$('#Country_countryname').val('');
$('#Country_recordstatus').prop('checked', false);
...
function editdata()
{
	...
	echo CHtml::ajax(array(
			...
			'success'=>"function(data)
			{
				if (data.status == 'success')
				{
				$('#Country_countryid').val(data.countryid);
$('#Country_countrycode').val(data.countrycode);
$('#Country_countryname').val(data.countryname);
if (data.recordstatus == 1) 
			{ 
				$('#Country_recordstatus').prop('checked', true); 
			} 
			else 
			{
				$('#Country_recordstatus').prop('checked', false);
...
<script type="text/javascript">
function downpdf() {
	window.open('country/downpdf?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function downexcel() {
	window.open('country/downexcel?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function searchdata()
...
$this->beginWidget(
    ...
    array(
        'title' => Catalogsys::model()->getCatalog('country'),
        'headerIcon' => 'th-list',
    	'padContent' => false,
