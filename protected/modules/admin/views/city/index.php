...
$this->breadcrumbs=array(
	...
	Catalogsys::model()->getCatalog('city'));
...
$('#City_cityname').val('');
$('#provincename').val('');
$('#City_recordstatus').prop('checked', false);
...
function editdata()
{
	...
	echo CHtml::ajax(array(
			...
			'success'=>"function(data)
			{
				if (data.status == 'success')
				{
				$('#City_cityid').val(data.cityid);
$('#City_provinceid').val(data.provinceid);
$('#City_citycode').val(data.citycode);
$('#City_cityname').val(data.cityname);
$('#provincename').val(data.provincename);
if (data.recordstatus == 1) 
			{ 
				$('#City_recordstatus').prop('checked', true); 
			} 
			else 
			{
				$('#City_recordstatus').prop('checked', false);
...
<script type="text/javascript">
function downpdf() {
	window.open('city/downpdf?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function downexcel() {
	window.open('city/downexcel?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function searchdata()
...
$this->beginWidget(
    ...
    array(
        'title' => Catalogsys::model()->getCatalog('city'),
        'headerIcon' => 'th-list',
    	'padContent' => false,
...
array(
        'id'=>'uploadFile',
        'config'=>array(
               'action'=>Yii::app()->createUrl('city/upload'),
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    array(
      ...
      'id'=>'ids',
    ),array('name'=>'cityid', 'value'=>'$data->cityid','htmlOptions'=>array('width'=>'1%')),array(
				'name'=>'provinceid',
				'type'=>'raw',
         'value'=>'($data->province!==null)?$data->province->provincename:""'
			),array(
				'name'=>'citycode',
				'type'=>'raw',
         'value'=>'$data->citycode'
