...
$this->breadcrumbs=array(
	...
	Catalogsys::model()->getCatalog('menuaccess'));
...
$('#Menuaccess_parentid').val('');
$('#Menuaccess_moduleid').val('');
$('#parentname').val('');
$('#modulename').val('');
$('#Menuaccess_sortorder').val('');
$('#Menuaccess_recordstatus').prop('checked', false);
...
$('#Menuaccess_parentid').val(data.parentid);
$('#Menuaccess_moduleid').val(data.moduleid);
$('#parentname').val(data.parentname);
$('#modulename').val(data.modulename);
$('#Menuaccess_sortorder').val(data.sortorder);
if (data.recordstatus == 1) 
			{ 
				$('#Menuaccess_recordstatus').prop('checked', true); 
			} 
			else 
			{
				$('#Menuaccess_recordstatus').prop('checked', false);
...
<script type="text/javascript">
function downpdf() {
	window.open('menuaccess/downpdf?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function downexcel() {
	window.open('menuaccess/downexcel?id='+$.fn.yiiGridView.getSelection("datagrid"));
}
</script>
<script type="text/javascript">
function searchdata()
...
$this->beginWidget(
    ...
    array(
        'title' => Catalogsys::model()->getCatalog('menuaccess'),
        'headerIcon' => 'th-list',
    	'padContent' => false,
...
array(
        'id'=>'uploadFile',
        'config'=>array(
               'action'=>Yii::app()->createUrl('menuaccess/upload'),
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    array(
      ...
      'id'=>'ids',
    ),array('name'=>'menuaccessid', 'value'=>'$data->menuaccessid','htmlOptions'=>array('width'=>'1%')),array(
				'name'=>'menuname',
				'type'=>'raw',
         'value'=>'$data->menuname'
			),array(
         ...
         'value'=>'$data->menuicon'
				...
				'name'=>'parentid',
				'type'=>'raw',
         'value'=>'($data->menuparent!==null)?$data->menuparent->description:""'
			),array(
				'name'=>'moduleid',
				'type'=>'raw',
         'value'=>'($data->module!==null)?$data->module->moduledesc:""'
			),array(
				'name'=>'sortorder',
				'type'=>'raw',
