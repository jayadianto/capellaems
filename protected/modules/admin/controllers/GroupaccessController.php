...
class GroupaccessController extends Controller
{
		...
		public function actionUpdate()
	{
		...
		{
			$id=$_POST['id'];
			$model=Groupaccess::model()->findByPk((int)$id[0]);
			if ($model != null)
				{
	...
	public function actionWrite()
	{
	  if(isset($_POST['Groupaccess']))
	  {
			$messages = $this->ValidateData(
							...
							array($_POST['Groupaccess']['groupname'],'emptygroupname','emptystring'),
					)
			);
			if ($messages == '') 
			{
				...
				try
				{
					...
					if ((int)$_POST['Groupaccess']['groupaccessid'] > 0)
					{
						...
						$command->bindvalue(':vid',$_POST['Groupaccess']['groupaccessid'],PDO::PARAM_STR);
					}
					else
					{
					...
					$command->bindvalue(':vgroupname',$_POST['Groupaccess']['groupname'],PDO::PARAM_STR);
$command->bindvalue(':vrecordstatus',$_POST['Groupaccess']['recordstatus'],PDO::PARAM_STR);
		...
		public function actionDelete()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				...
				$sql = 'call Deletegroupaccess(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
	...
	public function actionDownPDF()
	{
	  ...
	  $sql = "select groupname,recordstatus				from Groupaccess a ";
		if ($_GET['id'] !== '') {
				$sql = $sql . "where a.groupaccessid in (".$_GET['id'].")";
		}
		$command=$this->connection->createCommand($sql);
		$dataReader=$command->queryAll();

		//masukkan judul
		$this->pdf->title='Groupaccess List';
		$this->pdf->AddPage('P');
	...
	public function actionDownExcel()
	{
		...
		header('Content-Disposition: attachment;filename="Groupaccess.xlsx"');
		...
		if (($handle = fopen($this->filename, "r")) !== FALSE) 
		{
			while (($data = fgetcsv($handle, 2000, "^")) !== FALSE) 
			{
				if ($row>1)
				{
					...
					try
					{
												$sql = 'call UploadGroupaccess(:vid,:vgroupname,:vrecordstatus,:vcreatedby)';
						$command=$connection->createCommand($sql);
						$command->bindvalue(':vid',$data[0],PDO::PARAM_STR);
						$command->bindvalue(':vgroupname',$data[1],PDO::PARAM_STR);
