...
class UserfavController extends Controller
{
		...
		public function actionUpdate()
	{
		...
		{
			$id=$_POST['id'];
			$model=Userfav::model()->findByPk((int)$id[0]);
			if ($model != null)
				{
...
'username'=>($model->useraccess!==null)?$model->useraccess->username:"",
...
'menuname'=>($model->menuaccess!==null)?$model->menuaccess->menuname:"",
  ...
  {
		$this->DeleteLockCloseForm($this->menuname, $_POST['Userfav'], $_POST['Userfav']['userfavid']);
  }  
	...
	public function actionWrite()
	{
	  if(isset($_POST['Userfav']))
	  {
			$messages = $this->ValidateData(
							...
							array($_POST['Userfav']['useraccessid'],'emptyusername','emptystring'),
							array($_POST['Userfav']['menuaccessid'],'emptymenuname','emptystring'),
					)
			);
			if ($messages == '') 
			{
				...
				try
				{
					...
					if ((int)$_POST['Userfav']['userfavid'] > 0)
					{
						$sql = 'call UpdateUserfav(:vid,:vuseraccessid,:vmenuaccessid,:vcreatedby)';
						$command=$connection->createCommand($sql);
						$command->bindvalue(':vid',$_POST['Userfav']['userfavid'],PDO::PARAM_STR);
					}
					else
					{
					...
					$command->bindvalue(':vuseraccessid',$_POST['Userfav']['useraccessid'],PDO::PARAM_STR);
$command->bindvalue(':vmenuaccessid',$_POST['Userfav']['menuaccessid'],PDO::PARAM_STR);
					$command->bindvalue(':vcreatedby', Yii::app()->user->name,PDO::PARAM_STR);
					$command->execute();
					$transaction->commit();
					$this->DeleteLock($this->menuname, $_POST['Userfav']['userfavid']);
					$this->GetMessage('success','insertsuccess');
			  }
				catch (Exception $e)
				{
		...
		public function actionDelete()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				...
				$sql = 'call Deleteuserfav(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
	...
	public function actionDownPDF()
	{
	  ...
	  $sql = "select username,menuname				
		...
		foreach($dataReader as $row1)
		{
		  ...
		  $this->pdf->row(array($row1['username'],$row1['menuname']));
		}
	...
	public function actionDownExcel()
	{
		...
		$excel->setActiveSheetIndex(0)
					...
					->setCellValueByColumnAndRow(1,1,'Menu Name');
		foreach($dataReader as $row1)
		{
			  $excel->setActiveSheetIndex(0)
		...
		if (($handle = fopen($this->filename, "r")) !== FALSE) 
		{
			while (($data = fgetcsv($handle, 2000, "^")) !== FALSE) 
			{
				if ($row>1)
				{
					...
					try
					{
												$sql = 'call UploadUserfav(:vid,:vuseraccessid,:vmenuaccessid,:vcreatedby)';
						$command=$connection->createCommand($sql);
						$command->bindvalue(':vid',$data[0],PDO::PARAM_STR);
						$command->bindvalue(':vuseraccessid',$data[1],PDO::PARAM_STR);
