<div class="form-group">
	...
	$country=new Snro('searchwstatus');
			$country->unsetAttributes();  // clear any default values
			if(isset($_GET['Snro']))
			$country->attributes=$_GET['Snro'];
	...
	'cssFile' => Yii::app()->theme->baseUrl . '/css/gridview.css',				
				'columns'=>array(
					...
					/* Here is The Button that will send the Data to The MAIN FORM */
						...
						"id" => "send_product",
						"onClick" => "$(\"#'.$this->IDDialog.'\").dialog(\"close\"); $(\"#'.$this->ColField.'\").val(\"$data->description\"); $(\"#'.$this->Prefix.'_'.$this->IDField.'\").val(\"$data->snroid\");
						'.$this->onaftersign.'
				"))',
						),
		array('name'=>'snroid', 'value'=>'$data->snroid','htmlOptions'=>array('width'=>'1%')),
					'description',
					),
			));
