...
class IdentitytypeController extends Controller
{
		...
		public function actionUpdate()
	{
		...
		{
			$id=$_POST['id'];
			$model=Identitytype::model()->findByPk((int)$id[0]);
			if ($model != null)
				{
	...
	public function actionWrite()
	{
	  if(isset($_POST['Identitytype']))
	  {
			$messages = $this->ValidateData(
							...
							array($_POST['Identitytype']['identitytypename'],'emptyidentitytypename','emptystring'),
					)
			);
			if ($messages == '') 
			{
				...
				try
				{
					...
					if ((int)$_POST['Identitytype']['identitytypeid'] > 0)
					{
						...
						$command->bindvalue(':vid',$_POST['Identitytype']['identitytypeid'],PDO::PARAM_STR);
					}
					else
					{
					...
					$command->bindvalue(':videntitytypename',$_POST['Identitytype']['identitytypename'],PDO::PARAM_STR);
$command->bindvalue(':vrecordstatus',$_POST['Identitytype']['recordstatus'],PDO::PARAM_STR);
		...
		public function actionDelete()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				...
				$sql = 'call Deleteidentitytype(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
	...
	public function actionDownPDF()
	{
	  ...
	  $sql = "select identitytypename,recordstatus				from Identitytype a ";
		if ($_GET['id'] !== '') {
		...
		//masukkan judul
		$this->pdf->title='Identitytype List';
		$this->pdf->AddPage('P');
		//masukkan posisi judul
		$this->pdf->colalign = array('C');
		//masukkan colom judul
		$this->pdf->colheader = array('Identity Type');
	...
	public function actionDownExcel()
	{
		...
		$excel->setActiveSheetIndex(0)
					->setCellValueByColumnAndRow(0,1,'Identity Type');
		foreach($dataReader as $row1)
		{
			  $excel->setActiveSheetIndex(0)
					->setCellValueByColumnAndRow(0, $i+1, $row1['identitytypename']);
		$i+=1;
		}
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="Identitytype.xlsx"');
		...
		if (($handle = fopen($this->filename, "r")) !== FALSE) 
		{
			while (($data = fgetcsv($handle, 2000, "^")) !== FALSE) 
			{
				if ($row>1)
				{
					...
					try
					{
												$sql = 'call UploadIdentitytype(:vid,:videntitytypename,:vrecordstatus,:vcreatedby)';
						$command=$connection->createCommand($sql);
						$command->bindvalue(':vid',$data[0],PDO::PARAM_STR);
						$command->bindvalue(':videntitytypename',$data[1],PDO::PARAM_STR);
