...
class AddresstypeController extends Controller
{
		...
		public function actionUpdate()
	{
		...
		{
			$id=$_POST['id'];
			$model=Addresstype::model()->findByPk((int)$id[0]);
			if ($model != null)
				{
...
'recordstatus'=>$model->recordstatus,
  ...
  {
		$this->DeleteLockCloseForm($this->menuname, $_POST['Addresstype'], $_POST['Addresstype']['addresstypeid']);
  }  
	...
	public function actionWrite()
	{
	  if(isset($_POST['Addresstype']))
	  {
			$messages = $this->ValidateData(
							...
							array($_POST['Addresstype']['addresstypename'],'emptyaddresstypename','emptystring'),
					)
			);
			if ($messages == '') 
			{
				...
				try
				{
					...
					if ((int)$_POST['Addresstype']['addresstypeid'] > 0)
					{
						...
						$command->bindvalue(':vid',$_POST['Addresstype']['addresstypeid'],PDO::PARAM_STR);
					}
					else
					{
					...
					$command->bindvalue(':vaddresstypename',$_POST['Addresstype']['addresstypename'],PDO::PARAM_STR);
$command->bindvalue(':vrecordstatus',$_POST['Addresstype']['recordstatus'],PDO::PARAM_STR);
		...
		public function actionDelete()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				...
				$sql = 'call Deleteaddresstype(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
	...
	public function actionDownPDF()
	{
	  ...
	  $sql = "select addresstypename,recordstatus				from Addresstype a ";
		if ($_GET['id'] !== '') {
		...
		//masukkan judul
		$this->pdf->title='Addresstype List';
		$this->pdf->AddPage('P');
		//masukkan posisi judul
		$this->pdf->colalign = array('C');
		//masukkan colom judul
		$this->pdf->colheader = array('Address Type');
	...
	public function actionDownExcel()
	{
		...
		$excel->setActiveSheetIndex(0)
					->setCellValueByColumnAndRow(0,1,'Address Type');
		foreach($dataReader as $row1)
		{
			  $excel->setActiveSheetIndex(0)
					->setCellValueByColumnAndRow(0, $i+1, $row1['addresstypename']);
		$i+=1;
		}
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="Addresstype.xlsx"');
		...
		if (($handle = fopen($this->filename, "r")) !== FALSE) 
		{
			while (($data = fgetcsv($handle, 2000, "^")) !== FALSE) 
			{
				if ($row>1)
				{
					...
					try
					{
												$sql = 'call UploadAddresstype(:vid,:vaddresstypename,:vrecordstatus,:vcreatedby)';
						$command=$connection->createCommand($sql);
						$command->bindvalue(':vid',$data[0],PDO::PARAM_STR);
						$command->bindvalue(':vaddresstypename',$data[1],PDO::PARAM_STR);
