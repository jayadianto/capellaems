...
class CrudCode extends CCodeModel
{
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			array('model, modeldetail, isdetail,controller', 'filter', 'filter'=>'trim'),
			array('model, controller, baseControllerClass', 'required'),
			array('model,modeldetail', 'match', 'pattern'=>'/^\w+[\w+\\.]*$/', 'message'=>'{attribute} should only contain word characters and dots.'),
			...
			array('model,modeldetail,isdetail', 'validateModel'),
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			'model'=>'Model Class',
			'modeldetail'=>'Model Detail Class',
			'controller'=>'Controller ID',
			'baseControllerClass'=>'Base Controller Class',
			'isdetail'=>'Is Master-Detail ?'
		));
	}
	...
	public function validateModel($attribute,$params)
	{
		...
		if(!is_string($class) || !$this->classExists($class))
			$this->addError('modeldetail', "Class '{$this->model}' does not exist or has syntax error.");
		else if(!is_subclass_of($class,'CActiveRecord'))
			$this->addError('modeldetail', "'{$this->model}' must extend from CActiveRecord.");
		else
		{
			$tabledetail=CActiveRecord::model($class)->tableSchema;
			if($table->primaryKey===null)
				$this->addError('modeldetail',"Table '{$tabledetail->name}' does not have a primary key.");
			else if(is_array($table->primaryKey))
				$this->addError('modeldetail',"Table '{$tabledetail->name}' has a composite primary key which is not supported by crud generator.");
			else
			{
